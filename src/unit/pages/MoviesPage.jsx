...
function MoviesPage() {
  ...
  const fetchMovies = async ({ queryKey }) => {
    ...
    const response = await authAxios.get(
      `${import.meta.env.VITE_API_BASE_URL}/api/movies/all`,
      {
        params: {
          page,
          limit: 10,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
        ...
        <Label className="text-white">Release Year Range</Label>
        <div className="flex flex-col sm:flex-row gap-4 !mt-0">
          <div className="flex-1">
            <Select value={year[0].toString()} onValueChange={(value) => setYear([Number.parseInt(value), year[1]])}>
              <SelectTrigger id="yearFrom" className="w-full">
                <SelectValue placeholder="Select year" />
              </SelectTrigger>
              <SelectContent>
                {Array.from({ length: 2024 - 1900 }, (_, i) => 1900 + i).map((y) => (
          ...
          <div className="flex-1">
            <Select value={year[1].toString()} onValueChange={(value) => setYear([year[0], Number.parseInt(value)])}>
              <SelectTrigger id="yearTo" className="w-full">
                <SelectValue placeholder="Select year" />
              </SelectTrigger>
              <SelectContent>
                {Array.from({ length: 2024 - year[0] }, (_, i) => year[0] + i).map((y) => (
                  <SelectItem key={y} value={y.toString()}>
                    {y}
                  </SelectItem>
                ))}
        ...
        <div>
          <Label htmlFor="rating" className="text-white">Minimum Rating</Label>
          <Select
